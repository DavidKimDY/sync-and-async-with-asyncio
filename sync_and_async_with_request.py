...
async def async_main():
    ...
    tasks = []
    for i in range(10):
        task = asyncio.ensure_future(async_pause(i))
        tasks.append(task)
...
async def sync_to_async_main():
    ...
    for i in range(10):
        pause = partial(sync_pause, text=i)
        task = loop.run_in_executor(None, pause)
        tasks.append(task)
    await asyncio.gather(*tasks)
    print(f'\nTaken time : {time.time() - start}')
